...
from local_config import IMAGENET_PATH as IMAGENET_PATH
...
i=3
for warmup_lr, warmup_epochs in [(1e-7, 15), (1e-6, 15)]:
    config = {
        "batch-size": 512,
        ...
        "tag": f"{base_tag}_class_map_{i}_warmlr_{warmup_lr}_warmepo_{warmup_epochs}",
    }
...
config = {
    ...
    "model": 'mobilenetv3_small_100',
    "class-map": f"class_maps/class_maps_five_in_place/class_map{i}.txt",
    "tag": f"{base_tag}_class_map_{i}_lr_noise",
    "lr-noise": "0.42 0.9",
}
configs.append([config, None])
