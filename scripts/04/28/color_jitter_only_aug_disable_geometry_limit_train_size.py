...
for color_jitter in [0.4]:
    config = {
        ...
        "tag": f"{base_tag}_day_color_jitter_{color_jitter}",
    }
    configs.append([config, None])

for color_jitter in [0.4]:
    config = {
        "black-white-jitter": "parameter_without_value",
        "color-jitter": color_jitter,
        "tag": f"{base_tag}_day_black_white_jitter_{color_jitter}",
    }
    configs.append([config, None])
...
for color_jitter in [0.4]:
    config = {
        ...
        "tag": f"{base_tag}_day_black_white_jitter_{color_jitter}_grayscale",
    }
    configs.append([config, None])
...
for color_jitter in [0.4]:
    config = {
        "color-jitter": color_jitter,
        "to_grayscale": "parameter_without_value",
        "tag": f"{base_tag}_day_color_jitter_{color_jitter}_grayscale",
    }
    configs.append([config, None])

for color_jitter in [0.4]:
    config = {
        "tag": f"{base_tag}_day_color_jitter_{color_jitter}_random_invert_p_0.5_grayscale",
        "to_grayscale": "parameter_without_value",
        "random_invert_p": 0.5,
        "color-jitter": color_jitter
    }
...
for color_jitter in [0.2, 0.6, 0.8]:
    config = {
        "tag": f"{base_tag}_day_color_jitter_{color_jitter}_random_invert_p_0.5",
        "to_grayscale": "parameter_without_value",
        "random_invert_p": 0.5,
